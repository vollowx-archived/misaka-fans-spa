...
export default class ConCard extends HTMLElement {
  ...
  connectedCallback() {
    this.innerHTML = /* html */ `
      <div class="headline">
        <div class="avatar">
          <img src="https://avatars.githubusercontent.com/${this.getAttribute('gh-name')}?s=200&amp;v=4" alt="${this.getAttribute('name')}">
        </div>
        <div class="info">
          <p class="name title-large">${this.getAttribute('name')}</p>
          <a class="icon-button standard" href="https://github.com/${this.getAttribute(
            'gh-name'
          )}" title="TA 的 GitHub 主页：https://github.com/${this.getAttribute('gh-name')}">
